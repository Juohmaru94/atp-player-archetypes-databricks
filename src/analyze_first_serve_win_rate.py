...
OUTPUT_FILE = PROCESSED_DATA_DIR / "first_serve_points_won_win_rate_buckets.csv"
...
def main() -> None:
    ...
    bins = [0.0, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 1.01]
    labels = [
        "<55%",
        "55-60%",
        "60-65%",
    ...
    df["first_serve_win_bucket"] = pd.cut(
        df["first_serve_win_rate"],
        bins=bins,
        labels=labels,
        include_lowest=True,
        right=False,
    ...
    summary = (
        df.groupby("first_serve_win_bucket", observed=False, as_index=False)
        .agg(
